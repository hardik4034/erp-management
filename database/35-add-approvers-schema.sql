-- SQL Migration Script: Add Approver Fields to Employees
USE HRMS;
GO

-- 1. Add new columns if they don't exist
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'ExpenseApprover')
BEGIN
    ALTER TABLE Employees ADD ExpenseApprover NVARCHAR(255);
END
GO

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'ShiftRequestApprover')
BEGIN
    ALTER TABLE Employees ADD ShiftRequestApprover NVARCHAR(255);
END
GO

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'LeaveApprover')
BEGIN
    ALTER TABLE Employees ADD LeaveApprover NVARCHAR(255);
END
GO

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'SecondLeaveApprover')
BEGIN
    ALTER TABLE Employees ADD SecondLeaveApprover NVARCHAR(255);
END
GO

-- 2. Update Stored Procedures

-- Update sp_CreateEmployee to support new approver fields
CREATE OR ALTER PROCEDURE sp_CreateEmployee
    ...
    @ExpenseApprover NVARCHAR(255) = NULL,
    @ShiftRequestApprover NVARCHAR(255) = NULL,
    @LeaveApprover NVARCHAR(255) = NULL,
    @SecondLeaveApprover NVARCHAR(255) = NULL
AS
BEGIN
    ...
    INSERT INTO Employees (
        ...
        ExpenseApprover, ShiftRequestApprover, LeaveApprover, SecondLeaveApprover
    )
    VALUES (
        @EmployeeCode, @FirstName, @LastName, @Email, @Phone, @DateOfJoining, @DepartmentId, @DesignationId,
        @Salutation, @Password, @Country, @Gender, @DateOfBirth, @ReportingTo, @Language, @UserRole,
...
-- Update sp_UpdateEmployee to support new approver fields
...
BEGIN
    ...
    SET FirstName = @FirstName,
        ...
        ShiftRequestApprover = @ShiftRequestApprover,
        LeaveApprover = @LeaveApprover,
        SecondLeaveApprover = @SecondLeaveApprover
