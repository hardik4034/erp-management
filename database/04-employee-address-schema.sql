-- SQL Migration Script: Split Address into Permanent and Temporary Address
USE HRMS;
GO

-- 1. Add new columns if they don't exist
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'PermanentAddress')
BEGIN
    ALTER TABLE Employees ADD PermanentAddress NVARCHAR(500);
END
GO

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employees') AND name = 'TemporaryAddress')
BEGIN
    ALTER TABLE Employees ADD TemporaryAddress NVARCHAR(500);
END
GO

-- 2. Migrate existing data from 'Address' to 'PermanentAddress'
UPDATE Employees
SET PermanentAddress = Address
WHERE PermanentAddress IS NULL AND Address IS NOT NULL;
GO

-- 3. Update Stored Procedures

-- Re-create sp_CreateEmployee with new parameters
...
BEGIN
    ...
    INSERT INTO Employees (
        ...
        EmploymentType, MaritalStatus, BusinessAddress, Address -- keep Address for backward compatibility
    )
    VALUES (
        @EmployeeCode, @FirstName, @LastName, @Email, @Phone, @DateOfJoining, @DepartmentId, @DesignationId,
        @Salutation, @Password, @Country, @Gender, @DateOfBirth, @ReportingTo, @Language, @UserRole,
...
-- Re-create sp_UpdateEmployee with new parameters
...
BEGIN
    ...
    SET FirstName = @FirstName,
        ...
        Address = @PermanentAddress -- keep Address for backward compatibility
